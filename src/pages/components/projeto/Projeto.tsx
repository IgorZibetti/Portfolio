      ...
      <div className="2xl:w-[1100px] 2xl:h-[640px] p-2 shadow-2xl" ref={modalRef}>
        ...
        <div className="fixed justify-center items-center flex flex-col">
          ...
          <div className="flex justify-around items-center w-full">
            <button
              className="w-full p-2 bg-black text-white hover:bg-sky-700 font-black text-2xl"
              onClick={onClose}
            >
              FECHAR
            </button>
            <div className="w-1"></div>
            <Link href={content.link} target="_blank" className="flex w-full justify-center items-center p-2 bg-black text-white hover:bg-sky-700 font-black text-2xl">
              CODIGO
            </Link>
          </div>
        </div>
...
const App: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<{ title: string; description: string; videoSrc: string; link: string } | null>(null);

  const openModal = (item: { title: string; description: string; videoSrc: string; link: string }) => {
      ...
      <div id='projetos' className='md:mt-[400px] 2xl:mt-[150px]'>
        ...
        <div className='md:flex gap-5 grid justify-around items-center mb-[150px]'>
          ...
          {itemList.map((item, index) => (
            <motion.div key={item.title} whileHover={{ scale: 1.2 }} className='relative cursor-pointer' onClick={() => openModal(item)}>
              <Image
                width={450}
                height={350}
